//! Lossless predictive Rice codec layered on the VKA1 packet flags.
//!
//! Wire contract: `audio/protocol/audio_v1.json`, section `lossless_rice`.
//! An audio-data packet whose flags carry [`LOSSLESS_RICE_FLAG`] holds a
//! compressed payload; [`normalize_lossless_audio_notification`] inflates it
//! back to raw PCM before the shared packet parser runs, so the plain
//! PCM/session contract remains byte-for-byte unchanged.
//!
//! The decoder covers all three codec versions. The encoder mirrors the
//! host-side V1/V2 parameter search; embedded encoders (see
//! `audio/embedded/c/denzic_audio_lossless_v1`) additionally emit V3.

use crate::generated;
use crate::{parse_packet, PacketType};

pub const LOSSLESS_RICE_FLAG: u8 = generated::LOSSLESS_RICE_PACKET_FLAG;
// Encoder-tuning constants consumed by the embedded codec; re-exported so
// hosts can inspect the negotiated parameter space.
pub use generated::{
    LOSSLESS_RICE_V2_PARAMETER_K_SPAN, LOSSLESS_RICE_V3_MAX_PREDICTOR,
    LOSSLESS_RICE_V3_PREDICTOR_SAMPLE_STRIDE,
};
const HEADER_BYTES: usize = crate::HEADER_LEN;
const PAYLOAD_LEN_OFFSET: usize = 16;
const RICE_VERSION_V1: u8 = generated::LOSSLESS_RICE_VERSION_V1;
const RICE_VERSION_V2: u8 = generated::LOSSLESS_RICE_VERSION_V2;
const RICE_VERSION_V3: u8 = generated::LOSSLESS_RICE_VERSION_V3;
const RICE_HEADER_BYTES: usize = generated::LOSSLESS_RICE_HEADER_BYTES;
const RICE_MAX_K: u8 = generated::LOSSLESS_RICE_MAX_K;
const MAX_ZIGZAG_RESIDUAL: u32 = generated::LOSSLESS_RICE_MAX_ZIGZAG_RESIDUAL;
const RICE_PREDICTOR_FIRST_ORDER: u8 = generated::LOSSLESS_RICE_PREDICTOR_FIRST_ORDER;
const RICE_PREDICTOR_SECOND_ORDER: u8 = generated::LOSSLESS_RICE_PREDICTOR_SECOND_ORDER;
const RICE_PREDICTOR_THIRD_ORDER: u8 = generated::LOSSLESS_RICE_PREDICTOR_THIRD_ORDER;
const RICE_PREDICTOR_FOURTH_ORDER: u8 = generated::LOSSLESS_RICE_PREDICTOR_FOURTH_ORDER;

pub fn normalize_lossless_audio_notification(notification: &[u8]) -> Result<Vec<u8>, String> {
    if notification.len() < 6
        || &notification[..crate::MAGIC.len()] != crate::MAGIC
        || notification[5] & LOSSLESS_RICE_FLAG == 0
    {
    ...
    normalized[5] &= !LOSSLESS_RICE_FLAG;
...
pub fn decode_lossless_predictive_rice(
    payload: &[u8],
    ...
    let (predictor, rice_k, seed_count) = match version {
        RICE_VERSION_V1 => (
            generated::LOSSLESS_RICE_V1_PREDICTOR,
            control,
            generated::LOSSLESS_RICE_V1_SEED_SAMPLES,
        ),
        RICE_VERSION_V2 => (
            control >> 4,
            control & RICE_MAX_K,
            generated::LOSSLESS_RICE_V2_SEED_SAMPLES,
        ),
        RICE_VERSION_V3 => (
            control >> 4,
            control & RICE_MAX_K,
    ...
    if !(RICE_PREDICTOR_FIRST_ORDER..=RICE_PREDICTOR_FOURTH_ORDER).contains(&predictor)
        || (version != RICE_VERSION_V3 && predictor > generated::LOSSLESS_RICE_V2_MAX_PREDICTOR)
    {
...
/// Encodes little-endian i16 PCM with the V1 or V2 codec. V1 pins the
/// second-order predictor and searches the full Rice parameter range; V2
/// additionally negotiates the first-order predictor. Unlike the embedded
/// encoder this host helper always emits a frame, even when it grows.
pub fn encode_lossless_predictive_rice(pcm: &[u8], version: u8) -> Result<Vec<u8>, String> {
    if pcm.len() < 4 || pcm.len() % 2 != 0 {
        return Err(format!(
            "lossless PCM byte count must be an even value of at least four, got {}",
            pcm.len()
        ));
    }
    if !matches!(version, RICE_VERSION_V1 | RICE_VERSION_V2) {
        return Err(format!(
            "host lossless Rice encoder supports versions {RICE_VERSION_V1} and {RICE_VERSION_V2}, got {version}"
        ));
    }
...
mod tests {
    ...
    fn codec_round_trips_speech_like_pcm_without_loss() {
        let samples: Vec<i16> = (0..112)
            ...
            .collect();
        let raw = pcm(&samples);
        let compressed = encode_lossless_predictive_rice(&raw, RICE_VERSION_V2).expect("encode");
    ...
    #[test]
    fn compressed_notification_is_normalized_before_packet_parser() {
        let mut notification = vec![0u8; HEADER_BYTES];
        notification[..4].copy_from_slice(crate::MAGIC);
        notification[4] = PacketType::AudioData.wire_value();
        notification[5] = LOSSLESS_RICE_FLAG;
        ...
        let normalized = normalize_lossless_audio_notification(&notification).expect("normalize");
    ...
    fn v2_first_order_predictor_fits_a_packet_that_v1_cannot() {
        ...
        let samples: Vec<i16> = (0..200)
            ...
            .collect();
        let raw = pcm(&samples);
        let v1 = encode_lossless_predictive_rice(&raw, RICE_VERSION_V1).expect("v1 encode");
        let v2 = encode_lossless_predictive_rice(&raw, RICE_VERSION_V2).expect("v2 encode");
